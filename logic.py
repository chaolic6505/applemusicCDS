...
def track_get_info(LAST_FM_API_key, ARTIST_NAME_COVER_SIZE, TRACK_TITLE, requests, ALBUM_COVER_SIZE):
    ...
    GENRE = "Not Specified" if result['track']['toptags']['tag'][ALBUM_COVER_SIZE]['name'] == '' else result['track']['toptags']['tag'][ALBUM_COVER_SIZE]['name']
    print(GENRE)
    return GENRE
...
def album_cover_get_info(LAST_FM_API_key, ARTIST_NAME_COVER_SIZE, TRACK_TITLE, requests, ALBUM_COVER_SIZE):
    ...
    print(result)
    Album_Cover= result['album']['image'][ALBUM_COVER_SIZE]['#text']
